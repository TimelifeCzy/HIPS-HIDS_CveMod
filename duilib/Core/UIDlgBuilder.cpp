                                   ...
                                   CPaintManagerUI* pManager, CControlUI* pParent)
{
    ...
    if( HIWORD(xml.m_lpstr) != NULL ) {
...
#ifdef _DEBUG
			DUITRACE(_T("Create dialog:%s"), xml.m_lpstr);
#endif
            if (!m_xml.LoadFromFile(xml.m_lpstr)) {
#ifdef _DEBUG
				DUITRACE(_T("LoadFromFile error:%s"), xml.m_lpstr);
#endif
                return NULL;
            } 
        }
        ...
        if( hGlobal == NULL ) {
#ifdef _DEBUG
			DUITRACE(_T("LoadResource error:%s"), xml.m_lpstr);
#endif
    ...
    auto pControl = Create(pCallback, pManager, pParent);
    if (pControl == nullptr) {
#ifdef _DEBUG
		DUITRACE(_T("Create error:%s"), xml.m_lpstr);
#endif
    }
    return pControl;
}
...
{
#ifdef _DEBUG
	DUITRACE(_T("CDialogBuilder _Parse"));
#endif
...
#ifdef _DEBUG
		DUITRACE(_T("_Parse control:%s"), pstrClass);
#endif
