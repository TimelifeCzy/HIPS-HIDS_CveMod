...
//
//bool WriteFile(std::wstring& str)
//{
//    std::wstring data = str + std::wstring(L"\r\n");
//	WCHAR wszFilePath[MAX_PATH + 1] = { 0 };
//	GetModuleFileName(NULL, wszFilePath, MAX_PATH);
//	(wcsrchr(wszFilePath, L'\\'))[1] = 0;
//    std::wstring strFileNazme = std::wstring(wszFilePath) + std::wstring(L"log/duilib.log");
//	
//    FILE* file = _wfopen(strFileNazme.c_str(), L"w");
//	if (file == NULL) {  
//		return false;
//	}
//    char sztmp[512] = { 0 };
//    memcpy(sztmp, data.c_str(), data.length() * 2 > 511? 511 : data.length() * 2);
//	fwrite(sztmp, 1, sizeof(sztmp), file);
//    fclose(file);
//	return true;
//}
...
bool CMarkup::_Failed(LPCTSTR pstrError, LPCTSTR pstrLocation)
{
    ...
    //std::wstring strErrorMsg (pstrError);
    //WriteFile(strErrorMsg);
