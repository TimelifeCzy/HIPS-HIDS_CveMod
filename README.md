...
- `CveDriverPort` 单客户端模型，只有一个驱动实例。
- `CveMonitorPort` 单连接串行处理，如果未来要支持多个被注入进程同时上报，需要把服务端改成多连接 accept + worker 模型。
...
## 文章
- `https://bbs.kanxue.com/thread-266891-1.htm`
